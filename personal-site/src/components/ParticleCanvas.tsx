...
export default function ParticleCanvas() {
  ...
  useEffect(()=>{
    ...
    let W=0, H=0, cx=0, cy=0, rafId=0
    type Planet = {
      r:number;a:number;b:number;incl:number;speed:number;base:string;atmos:string;ring:boolean
      pal:typeof PALETTES[0];angle:number;x:number;y:number;ox:number;oy:number
      dvx:number;dvy:number;trail:{x:number;y:number}[];warpAlpha:number;rotAngle:number;rotSpeed:number
    }
    let planets:Planet[] = []
    ...
    function init(){
      ...
      planets = DEFS.map((d,i)=>{
        ...
        return{...d,pal:PALETTES[i],angle:d.phase,
    ...
    function drawPlanet(p:Planet){
      const{x,y,r,pal,ring,atmos,rotAngle}=p
      ctx.save(); ctx.translate(x,y)

      // atmosphere
      const ag=ctx.createRadialGradient(0,0,r*0.8,0,0,r*1.6)
      ag.addColorStop(0,atmos+'38'); ag.addColorStop(0.5,atmos+'14'); ag.addColorStop(1,atmos+'00')
      ...
      // clouds
      for(let c=0;c<3;c++){
        ctx.beginPath(); ctx.ellipse(r*0.1*c,-r*0.6+c*r*0.5,r*(0.4+c*0.15),r*0.06,c*0.3,0,Math.PI*2)
        ctx.fillStyle='rgba(255,255,255,0.07)'; ctx.fill()
      }
      // polar caps
      ctx.beginPath(); ctx.arc(0,-r*0.8,r*0.32,0,Math.PI*2); ctx.fillStyle='rgba(255,255,255,0.13)'; ctx.fill()
      ctx.beginPath(); ctx.arc(0,r*0.85,r*0.22,0,Math.PI*2); ctx.fillStyle='rgba(255,255,255,0.07)'; ctx.fill()
      // specular
      const sg=ctx.createRadialGradient(-r*0.3,-r*0.32,0,0,0,r)
      sg.addColorStop(0,'rgba(255,255,255,0.30)'); sg.addColorStop(0.35,'rgba(255,255,255,0.06)'); sg.addColorStop(1,'rgba(0,0,0,0.45)')
    ...
    function drawBH(x:number,y:number){
      ctx.save(); ctx.translate(x,y)
      for(let i=5;i>=1;i--){
        ctx.save(); ctx.rotate(i*0.28); ctx.scale(1,0.18+i*0.035)
        ctx.beginPath(); ctx.arc(0,0,BH_R*(1+i*0.52),0,Math.PI*2); ctx.arc(0,0,BH_R*(1+i*0.38),0,Math.PI*2,true)
        const hue=i<=2?`255,${160+i*10},${30+i*5}`:i===3?'220,90,15':'140,45,8'
        ctx.fillStyle=`rgba(${hue},${0.05+i*0.038})`; ctx.fill(); ctx.restore()
      }
      ...
      ctx.beginPath(); ctx.arc(0,0,BH_R,0,Math.PI*2); ctx.strokeStyle='rgba(255,210,90,0.45)'; ctx.lineWidth=1.5; ctx.stroke()
    ...
    function tick(now:number){
      ...
      // sun glow
      const sunG=ctx.createRadialGradient(cx,cy,0,cx,cy,55)
      sunG.addColorStop(0,'rgba(255,230,140,0.20)'); sunG.addColorStop(1,'rgba(255,150,0,0)')
      ctx.beginPath(); ctx.arc(cx,cy,55,0,Math.PI*2); ctx.fillStyle=sunG; ctx.fill()
      ...
      DEFS.forEach(d=>{
        ...
        for(let a=0;a<=Math.PI*2+0.01;a+=0.04){
          const ex=d.a*Math.cos(a),ey=d.b*Math.sin(a)
          const px=cx+ex*cI-ey*sI,py=cy+ex*sI+ey*cI
      ...
      planets.forEach(p=>{
        ...
        if(active){
          ...
          if(dist<inf&&dist>BH_R+p.r){
            ...
            p.dvx+=(nx*8000*f/Math.max(dist,12)+tx*5000*f/Math.max(dist,12))*dt*0.016
            p.dvy+=(ny*8000*f/Math.max(dist,12)+ty*5000*f/Math.max(dist,12))*dt*0.016
  ...
  return (
    <canvas
      ref={canvasRef}
      data-hero-canvas
      onMouseMove={e=>{ mouseRef.current={x:e.clientX,y:e.clientY,active:true} }}
